...
lm_model <- lm(
  math ~ gender + computer + internet + family_class,
  data = df_clean,
...
stan_data <- list(
  ...
  math = df_clean$math,      # dependent variable
...
saveRDS(fit, file = "./data/1128model_math_Weighted_fit.rds")

# Load the saved model
fit <- readRDS(file = "./data/1128model_math_Weighted_fit.rds")
...
ppc_dens_overlay(stan_data$math, y_rep[1:50, ])

# Or check using test statistics (mean, standard deviation, etc.)
ppc_stat(stan_data$math, y_rep, stat = "mean")
ppc_stat(stan_data$math, y_rep, stat = "sd")

# Calculate residuals
y_mean <- apply(y_rep, 2, mean)  # Mean prediction for each student
residuals <- stan_data$math - y_mean  # Residuals (observed - predicted)
