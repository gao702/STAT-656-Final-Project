...
lm_model <- lm(
  science ~ gender + computer + internet + family_class,
  data = df_clean,
...
stan_data <- list(
  ...
  science = df_clean$science,      # dependent variable
...
ppc_dens_overlay(stan_data$science, y_rep[1:50, ])

# Or check using test statistics (mean, standard deviation, etc.)
ppc_stat(stan_data$science, y_rep, stat = "mean")
ppc_stat(stan_data$science, y_rep, stat = "sd")

# Calculate residuals
y_mean <- apply(y_rep, 2, mean)  # Mean prediction for each student
residuals <- stan_data$science - y_mean  # Residuals (observed - predicted)
