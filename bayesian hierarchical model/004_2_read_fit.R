...
# Prepare the dataset
data_read <- data %>%
  select(country, school_id, student_id, gender, computer, internet, family_class, read) %>%
  mutate(
...
saveRDS(data_read,file = "./data/cleaned/cleaned_data_read.Rds")

schools <- unique(data_read[, c("school_id", "country")])  # Unique school-country pairs
...
data {
  ...
  vector[N] y;                   // Outcome variable (read scores)
...
"
read_model<- stan_model(model_code = read_code)

read_data <- list(
  N = nrow(data_read),
  J = length(unique(data_read$school_id)),
  K = length(unique(data_read$country)),
  school_id = as.numeric(factor(data_read$school_id)),
  country_id = schools$country_id,
  y = data_read$read,
  gender = as.numeric(data_read$gender == "female"), # Binary encoding
  family_class = as.numeric(data_read$family_class=="middle class"), # Assuming it's numeric
  computer = as.numeric(data_read$computer == "yes"), # Binary encoding
  internet = as.numeric(data_read$internet == "yes") # Binary encoding
)

fit <- sampling(read_model,
                data = read_data,
                iter = 10000,
                warmup = 2000,
                # init_r = 0.01,
...
saveRDS(fit,file = "./result/read_fit.Rds")
